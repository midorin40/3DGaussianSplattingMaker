...
function createAssetRecord(storage, projectId, file) {
  ...
  return {
    ...
    previewUrl: `/v2/uploads/${projectId}/${saved.storedName}`,
...
function serveUpload(storage, pathname, res) {
  const relativePath = pathname.replace(/^\/(?:v2\/)?uploads\//, "")
...
async function handleRequest(req, res) {
  ...
  if (req.method === "GET" && pathname === "/api/status") {
    sendJson(res, 200, {
      name: "GaussianSplattingMaker v2",
      version: "runtime-api",
      supports: {
        projectCrud: true,
        assetIngestion: true,
      ...
      storage: {
        dataRoot: storage.paths.v2DataRoot,
  ...
  if (req.method === "GET" && (pathname.startsWith("/uploads/") || pathname.startsWith("/v2/uploads/"))) {
    serveUpload(storage, pathname, res)
    return
  }

  if (req.method === "GET" && (pathname.startsWith("/exports/") || pathname.startsWith("/v2/exports/"))) {
...
server.listen(PORT, () => {
  ...
  console.log(`GaussianSplattingMaker v2 listening on http://localhost:${PORT}`)
  console.log(`V2 data root: ${paths.v2DataRoot}`)
})
...
function serveExport(reqPath, res) {
  const relativePath = reqPath.replace(/^\/(?:v2\/)?exports\//, "")
